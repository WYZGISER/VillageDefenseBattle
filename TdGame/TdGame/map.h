	...
	bool load(const std::string& path)
	{
		...
		while (std::getline(file,str_line))
		{
			...
			while (std::getline(str_stream, str_tile, ','))
			{
				...
				load_tile_from_stream(tile, str_tile);
			}
		}
...
private:
	...
	void load_tile_from_stream(Tile& tile, const std::string& str)
	{
		...
		tile.direction = (Tile::Direaction)((values.size() < 3 || values[2] < 0) ? 0 : values[2]);
		...
		for (int y = 0; y < get_height(); y++)
		{
			for (size_t x = 0; x < get_width(); x++)
			{
				...
				else
				{
					...
					spawner_route_pool[tile.special_flag] = Route(tile_map, SDL_Point{ static_cast<int>(x), y });
				}
			}
