...
public:
	GameManager()
	{
		...
		init_assert(!TTF_Init(), TTF_GetError());
		...
		window = SDL_CreateWindow(config->basic_template.window_title.c_str(),
			SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
			config->basic_template.window_width,
			config->basic_template.window_height,
			SDL_WINDOW_SHOWN);
		init_assert(window, u8"´°¿Ú´´½¨Ê§°Ü");
...
private:
	void init_assert(bool flag, const char* err_msg) {
		if (flag) return;
		SDL_ShowSimpleMessageBox(flag, u8"game false", err_msg, window);
		exit(-1);
	}
